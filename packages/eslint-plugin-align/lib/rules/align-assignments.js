 ...
 * @author Lucas Florio
 *
 * Vendored from eslint-plugin-align-assignments@1.1.2 (MIT, Copyright (c) Lucas
 * Florio). Upstream is unmaintained since 2022 and crashes on ESLint 9+.
 * Local changes: sourceCode accessor shim, meta.schema for the requiresOnly option.
 */
'use strict';
...
module.exports = {
  meta: {
    type:    'layout',
    docs:    {
      description: 'align assignment operators within a group'
    },
    fixable: 'code',
    // ESLint 9+ rejects rule options unless the rule declares a schema. Upstream
    // read options[0].requiresOnly without declaring one, so passing the option
    // is a hard config error there.
    schema:  [
      {
        type:                 'object',
        properties:           {
          requiresOnly: { type: 'boolean' }
        },
